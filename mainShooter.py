...
from DQL_Agent import DQL as DQL_CNN
...
if(ai_mode and not cnn) :
    ...
    while (game_count < 150):
        game_count += 1
        epsilon = epsilon - 1 # epsilon is decayed as the model is well trained
        ...
        while not crash:
            ...
            if(np.random.randint(0,1000) < epsilon ): # do a random move
    ...
    while (game_count < 150):
        ...
        while not crash:
            ...
            if (np.random.randint(0, 100) < epsilon):  # do a random move
                ...
                state_ = cv2.resize(state, (28,28))
                action = agent.model.predict(np.array([state_]).reshape(1,28,28,1))[0]
                predicted_move = np.argmax(action)
            ...
            all_sprites.draw(screen)
            # GET new state here
            new_state = agent.get_state(screen)
